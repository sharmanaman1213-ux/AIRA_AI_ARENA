...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if hasattr(self, "_incognito") and self._incognito:
            kwargs["args"].append("--incognito")
...
class _SessionRegistry:
    ...
    def _get_or_create(self, browser_name: str, incognito: bool = False) -> _BrowserSession:
        with self._lock:
            # Always recreate if requesting incognito specifically or if not exists
            if browser_name not in self._sessions or incognito:
                if browser_name in self._sessions:
                    self._sessions[browser_name].close()
                sess = _BrowserSession(browser_name, incognito=incognito)
                sess.start()
                self._sessions[browser_name] = sess
                print(f"[Registry] New session: {browser_name} (incognito: {incognito})")
