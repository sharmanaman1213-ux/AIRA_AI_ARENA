...
def _nvml_gpu_windows() -> float:
    ...
    try:
        ...
        dev = ctypes.c_void_p()
        getattr(_nvml_lib, "nvmlDeviceGetHandleByIndex_v2")(0, ctypes.byref(dev))
        util = _Util()
        getattr(_nvml_lib, "nvmlDeviceGetUtilizationRates")(dev, ctypes.byref(util))
        _nvml_ok = True
...
class HudCanvas(QWidget):
    ...
    def paintEvent(self, a0):
        ...
        p.drawArc(srect, int(self._scan * 16), (ex * 16))
        p.setPen(QPen(qcol(C.ACC, sa // 2), 1.5))
        p.drawArc(srect, int(self._scan2 * 16), (ex * 16))
...
class LogWidget(QTextEdit):
    ...
    def __init__(self, parent=None):
        ...
        self._pos     = 0
        self._tag     = "sys"
        self._ai_name_lc = "aira"   # updated when assistant name changes
        self._tmr = QTimer(self)
        self._tmr.timeout.connect(self._step)
    ...
    def _next(self):
        ...
        elif tl.startswith(_ai_pfx) or tl.startswith("aira:"): self._tag = "ai"
...
class FileDropZone(QWidget):
    ...
    def dragEnterEvent(self, a0: QDragEnterEvent):
        mime = a0.mimeData() if hasattr(a0, 'mimeData') else None
    ...
    def _browse(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Select a file for AIRA", str(Path.home()),
            "All Files (*.*);;"
...
class CustomizeOverlay(QWidget):
    ...
    saved = pyqtSignal(str, str, str, str)   # assistant_name, user_name, ui_color, mode
    _OW, _OH = 400, 500

    def __init__(self, assistant_name="AIRA", user_name="",
                 ui_color=DEFAULT_UI_COLOR, current_mode="Tech Mode", parent=None):
        super().__init__(parent)
        self.setAttribute(Qt.WidgetAttribute.WA_StyledBackground, True)
        self.setStyleSheet(f"""
            CustomizeOverlay {{
        ...
        lay.addWidget(_lbl("OPERATION MODE", 8, color=C.TEXT_DIM, align=Qt.AlignmentFlag.AlignLeft))
        self._mode_input = QComboBox()
        self._mode_input.addItems(["Tech Mode", "Study Mode"])
        self._mode_input.setCurrentText(current_mode)
        self._mode_input.setFont(QFont("Courier New", 10))
        self._mode_input.setFixedHeight(32)
        self._mode_input.setStyleSheet(f"background: {C.PANEL2}; color: {C.WHITE}; border: 1px solid {C.BORDER};")
        lay.addWidget(self._mode_input)
    ...
    def _save(self):
        ...
        try:
            ...
            hx = self._initial_color
        self.saved.emit(self._name_input.text().strip(),
                        self._user_input.text().strip(),
                        hx,
                        self._mode_input.currentText())
        self.hide()
...
class ClipboardPanel(QWidget):
    """Floating panel shown when text is copied — offers quick Aira actions."""
...
class RemoteKeyOverlay(QWidget):
    ...
    def mark_connected(self) -> None:
        ...
        self._timer_lbl.setText("Phone connected — AIRA ready")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        self._assistant_name: str = (_cfg.get("assistant_name") or "AIRA").strip()
        ...
        self.on_interrupt      = None   # callable: () -> None — stop AIRA mid-speech
        self._muted            = False
        self._current_file: str | None = None
    ...
    # ------------------------------------------------------------------
    @staticmethod
    def _build_aira_icon(out_path: Path) -> bool:
        """
        Render a AIRA arc-reactor icon at 4× resolution and downsample
        ...
        def _render(sz: int) -> PIL.Image.Image:
            ...
            return img.resize((sz, sz), PIL.Image.Resampling.LANCZOS)
    ...
    def _create_desktop_shortcut(self):
        ...
        ico_path = Path(__file__).resolve().parent / "config" / "aira.ico"
        if not ico_path.exists():
            self._build_aira_icon(ico_path)
        ...
        try:
            ...
            # ── macOS — proper .app bundle (no Terminal window) ───────────────
                ...
                (app / "Contents" / "Info.plist").write_text(
                    ...
                    '  <key>CFBundleExecutable</key><string>AIRA</string>\n'
                    '  <key>CFBundleIdentifier</key>'
                    '<string>com.aira.assistant</string>\n'
                ...
                if not png_path.exists() and ico_path.exists():
                    try:
                        ...
                        PIL.Image.open(ico_path).resize(
                            (256, 256), PIL.Image.Resampling.LANCZOS
    ...
    def _build_left_panel(self) -> QWidget:
        ...
        # Mode Switcher
        mode_hdr = QLabel("◈ OPERATION MODE")
        mode_hdr.setFont(QFont("Courier New", 7, QFont.Weight.Bold))
        mode_hdr.setStyleSheet(f"color: {C.ACC}; background: transparent; padding-bottom: 2px;")
        lay.addWidget(mode_hdr)
        
        self.mode_selector = QComboBox()
        self.mode_selector.addItems(["Tech Mode", "Study Mode"])
        self.mode_selector.setFont(QFont("Courier New", 8, QFont.Weight.Bold))
        self.mode_selector.setStyleSheet(f"background: {C.PANEL2}; color: {C.WHITE}; border: 1px solid {C.BORDER}; border-radius: 3px; padding: 3px;")
        lay.addWidget(self.mode_selector)
    ...
    def _check_autostart(self) -> bool:
        ...
        try:
            if _OS == "Windows":
                ...
                try:
                    winreg.QueryValueEx(key, "AIRA_AI")
                    return True
                ...
                return (Path.home() / "Library" / "LaunchAgents"
                        / "com.aira.assistant.plist").exists()
            else:
                return (Path.home() / ".config" / "autostart" / "aira.desktop").exists()
        except Exception:
            return False
    ...
    def _toggle_autostart(self):
        ...
        try:
            ...
            if _OS == "Windows":
                ...
                if currently_on:
                    winreg.DeleteValue(reg, "AIRA_AI")
                else:
                    pythonw = Path(sys.executable).parent / "pythonw.exe"
                    exe = str(pythonw if pythonw.exists() else sys.executable)
                    winreg.SetValueEx(reg, "AIRA_AI", 0, winreg.REG_SZ,
                                      f'"{exe}" "{script}"')
                winreg.CloseKey(reg)
            elif _OS == "Darwin":
                plist_dir = Path.home() / "Library" / "LaunchAgents"
                plist_dir.mkdir(parents=True, exist_ok=True)
                plist = plist_dir / "com.aira.assistant.plist"
                if currently_on:
                    ...
                    plist.write_text(
                        ...
                        '"http://www.apple.com/DTDs/PropertyList-1.0.dtd">\n'
                        '<plist version="1.0"><dict>\n'
                        '  <key>Label</key><string>com.aira.assistant</string>\n'
    ...
    def _open_customize(self):
        ...
        cw = self.centralWidget()
        ov = CustomizeOverlay(
            cfg.get("assistant_name", "AIRA") or "AIRA",
            cfg.get("user_name", ""),
            cfg.get("ui_color", "") or DEFAULT_UI_COLOR,
            self.mode_selector.currentText(),
            parent=cw,
        )
        ow, oh = CustomizeOverlay._OW, CustomizeOverlay._OH
    ...
    def _apply_name_update(self, name: str, user_name: str, ui_color: str = "", mode: str = "Tech Mode"):
        """Update all name/theme-dependent UI elements and persist to config."""
        self._assistant_name = name.strip() or "AIRA"
        display = self._assistant_name.upper()
        self.setWindowTitle(f"{display} — MARK XLIX")
        self._title_lbl.setText(display)
        if display in ("AIRA", "J.A.R.V.I.S"):
    ...
    def _on_setup_done(self, key: str, os_name: str):
        ...
        if self._overlay:
            ...
            self._overlay = None
        self._apply_state("LISTENING")
        self._assistant_name = _read_full_config().get("assistant_name", "AIRA") or "AIRA"
    ...
    def _setup_tray_icon(self):
        ...
        icon_path = str(CONFIG_DIR / "aira.ico")
        ...
        menu = QMenu()
        show_action = QAction("Open AIRA", self)
...
class AiraUI:
    ...
    def on_mode_changed(self):
        return getattr(self._win, "on_mode_changed", None)

    @on_mode_changed.setter
    def on_mode_changed(self, cb):
        setattr(self._win, 'on_mode_changed', cb)
        try:
            self._win.mode_selector.currentTextChanged.connect(
                lambda text: self._win.on_mode_changed(text) if hasattr(self._win, 'on_mode_changed') and self._win.on_mode_changed else None
            )
        except AttributeError:
            pass
    ...
    def current_mode(self) -> str:
        try:
            return self._win.mode_selector.currentText()
        except AttributeError:
            return "Tech Mode"